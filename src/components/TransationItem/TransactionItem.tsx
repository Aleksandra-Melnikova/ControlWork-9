...
const TransactionItem:React.FC<ITransactionItemProps > = ({date, name, type, amount, onDelete, id}) => {
  ...
  return (
    ...
    <div className={'border border-1 rounded-2 mt-3 p-3 d-flex justify-content-between align-items-center fs-3 '}>
      <span className={'ms-2 fs-3 text-secondary '}>{dayjs(date).format('DD.MM.YYYY HH:mm:ss')}</span>
      <span className={'ms-2 fs-3 ms-5 '}>{name}</span>
      <span className={'ms-auto'}>{type === 'expense'? `- ${amount}`: `+ ${amount}`}</span>
      <div className="me-3">
        <button
          type="button"
        ...
        >
          {isDeleteTransactionLoading  ? <div className={'ms-5 ps-2'}><Spinner/></div> : null}
        </button>
      </div>
    </div>
